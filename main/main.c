...
#include "esp_crt_bundle.h"      // ✅ for SSL root certs
#include "esp_sntp.h"            // ✅ for SNTP time sync
#include "nvs_flash.h"
...
#define FIRMWARE_VERSION "1.0.0"   // Initial version for manual flash

// Direct firmware URL (from GitHub Release)
...
void obtain_time(void) {
    ...
    if (timeinfo.tm_year < (2016 - 1900)) {
        ESP_LOGW(TAG, "Time not set via SNTP. SSL may fail.");
    } else {
...
void perform_ota_update(void) {
    ...
    esp_http_client_config_t config = {
        ...
        .crt_bundle_attach = esp_crt_bundle_attach  // ✅ Fix: attach certificate bundle
    };
...
void app_main(void) {
    ...
    while (1) {
        ...
        if (counter % 30 == 0) {
            ESP_LOGI(TAG, "Status: Running version %s - Cycles: %d", FIRMWARE_VERSION, counter);
        }
    }
}
